...
async function generatePlaylistDescription(
  ...
  try {
    ...
    const description = response.text().trim().slice(0, 300);
...
export async function POST(req: Request) {
  try {
    ...
    const startLocationTracks = await searchGenreBasedTracks(
      startLocationGenres,
      accessToken
    );
    const endLocationTracks = await searchGenreBasedTracks(
      endLocationGenres,
      accessToken
    );
